...
| Path | Purpose |
|------|---------|
| `beer_hall/` | Beer Hall OpenClaw digests (entries + feed) |
| `deploys/` | **Deploy/push ledger** — append-only audit trail of every class push / deploy by any agent (Sophia, Envoy, Deep Seek, Bionpact, Kimi, Claude) or governor. Companion to `agentic_ai_context/sops/DEPLOY_PUSH_SOP.md`. See `deploys/README.md`. |
| `scripts/append_deploy_record.py` | Ledger writer: validates identity/result, writes entries + feed. Dry-run by default (`--write` commits). |
...
scripts/
  ...
  append_deploy_record.py          # deploy/push ledger writer
beer_hall/
...
deploys/
  README.md           # deploy ledger schema + rules
  entries/            # deploy_<ISO-UTC>_<slug>.md + .json
  feed/manifest.json  # rebuildable index, newest first
  leases/             # soft-lock lease convention (pre-push)
```
...
## Deploy/push ledger entries

After any **class push / deploy** (clasp push, gas_deploy_project, EC2 deploy, prod-sync, etc.), append a record:

```bash
python3 scripts/append_deploy_record.py --agent sophia --target-type clasp \
  --target-id 1N6o00N9VtRK --action "clasp push --force" \
  --result success --evidence-url https://github.com/TrueSightDAO/tokenomics --write
```

See `deploys/README.md` and `agentic_ai_context/sops/DEPLOY_PUSH_SOP.md` for the full procedure (identity rules, lease step, failure handling).
